...
const UserCreateAvatarUploadMulter = multer({
    ...
    fileFilter: function(request: Request<any, any, RequestBody>, file, callback) {
        ...
        if (!request.body.email) {
            return callback(new Error('Email é necessário para criar usuário.'));
        }
...
export default function UserCreateAvatarUpload(
    request: Request,
    response: Response,
    next: NextFunction
) {
    const upload = UserCreateAvatarUploadMulter.single('avatar');
